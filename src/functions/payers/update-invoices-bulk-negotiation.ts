...
import { parseNegotiationUpdateProcessStatus, EnterpriseInvoiceNegotiationProcessStatus } from 'commons/enums/enterprise-invoice-negotiation-process-status.enum';
...
const originalHandler: APIGatewayProxyHandler = async (event: APIGatewayProxyEvent, context: Context) => {
    ...
    try {
        ...
        if (!bulkNegotiationId || bulkNegotiationId === undefined) throw new BadRequestException('SCF.LIBERA.249');
        ...
        await EnterpriseInvoiceNegotiationService.updateInvoiceBulkNegotiationById(+enterpriseId, +bulkNegotiationId, { status, newOffer }, +userId);
